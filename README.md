...
- ### 개선 방향
    - #### 포트포워딩 : 라즈베리파이와 사용자가 서로 다른 네트워크를 사용할 때에도 신호를 보내 제어할 수 있는 방법. 이것을 하지 못해 아쉽다.
    - #### 모터 속도 : 서보블라스터를 사용하니 일반적인 모터의 속도보다 훨씬 못미치는 걸 확인했다. 속도 제어를 사용자가 원하는 만큼 제어 가능하도록 못해 아쉽다.
    - #### 모터 고정 : 배식기와 모터 구조상 테이프나 본드로 제대로 부착하기 어려움이 있었다. 3d펜이나 글루건을 사용해 제대로 부착하지 못한 것이 아쉽다. 
